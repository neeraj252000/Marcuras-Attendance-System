# -*- coding: utf-8 -*-
"""
Created on Sun May 16 17:51:20 2021
...
def Choose_Algo():
    algo_chosen = algo_options.get()
    
    if(algo_chosen == 'LSB Algorithm'):
        LSB()
        
    elif(algo_chosen == 'Frequency Algorithm'):
        Frequency()
    return
...
def LSB():
    ...
    def Encode():
        ...
        data = (text_entry.get("1.0", "end-1c")) + delimeter
        data_length += 8 #adding 8 bits extra for delimeter
        print(data)
        ...
        for values in img1:
            for pixel in values:
                pixel_binary = [format(i, '08b') for i in pixel]
                r, g, b = pixel_binary
                if(data_encoded_count<data_length):
                    pixel[0] = int(r[:-1]+data_binary[data_encoded_count], 2)
                    data_encoded_count += 1
                if(data_encoded_count<data_length):
                    pixel[1] = int(g[:-1]+data_binary[data_encoded_count], 2)
                    data_encoded_count += 1
                if(data_encoded_count<data_length):
                    pixel[2] = int(b[:-1]+data_binary[data_encoded_count], 2)
                    data_encoded_count += 1
            if(data_encoded_count>=data_length):
                break
        ...
        stego_img_pil_resized = stego_img_pil.resize(Dim(stego_img_pil,
                                                         height=300))
        stego_img_pil_resized = ImageTk.PhotoImage(stego_img_pil_resized,
                                                   master = save_img_window)
        ...
        def Decode_Data():
            ...
            for values in img2:
                for pixel in values:
                    ...
                    binary_decoded_string += r[-1]
                    n+=1
                    if(n%8==0):
                        char_byte = binary_decoded_string[n-8:n]
...
def Frequency():
    ...
    def Upload_IMG():
        ...
        max_data = max_frequency//8
        max_data_label['text'] = "Max Data: " + str(max_data*8)
        return 0
    ...
    def Encode():
        global max_freq_intensity, img1
        global data_length
        if(data_length==0):
            tkinter.messagebox.showinfo("Empty Data", "PLease enter text")
            return
    ...
    root.mainloop()
main_root = Tk()
main_root.title("Steganography")
main_root.geometry("400x300")
main_root.configure(background='#474747')
#main_root.iconbitmap('icon.ico')
main_root.resizable(width = False, height=False)
    
main_title = Label(main_root, text="Image Steganography Tool", bg='#474747',
              fg='#FFFFFF', font=("Alegreya Sans Bold", 20))
main_title.pack()

choose_label = Label(main_root, text="Choose Algorithm", bg='#474747',
                     fg='#FFFFFF', font=("Alegreya Sans Bold", 15))
choose_label.pack()
choose_label.place(anchor = 'center', relx = 0.5, rely = 0.4)

algo_options = ttk.Combobox(main_root, width = 27)
                                
algo_options['values'] = ('LSB Algorithm', 'Frequency Algorithm')
algo_options.pack()
algo_options.place(anchor = "center", rely = 0.5, relx = 0.5)
algo_options.current(0)

submit_button = Button(main_root, text="Submit",
                       font=("Alegreya Sans Bold", 10),
                       fg = '#474747', bg = 'white',
                       command = Choose_Algo)
submit_button.pack()
submit_button.place(anchor = "center", rely = 0.8, relx = 0.5)
main_root.mainloop()